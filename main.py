...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-i", "--install", action="store_true", help="Installation Mode Only"
    )
    parser.add_argument(
        "-ip",
        "--host",
        type=str,
        help="Database ip Address (HTTPS is Not Supported Here)",
    )
    ...
    if database_env_valid is False:
        ...
        with open(database_env_path, "w") as file:
            print(
                "Please enter the following information associated with the database."
            )
            if args.host:
                file.write(f"HOST={args.host}\n")
            else:
                file.write(
                    f'HOST={input("Database IP Address (HTTPS is Not Supported Here): ")}\n'
                )
            if args.port:
    ...
    from backend.API.Endpoints.roof_load_combination_endpoint import (
    ...
    from backend.API.Endpoints.wall_load_combination_endpoint import (
