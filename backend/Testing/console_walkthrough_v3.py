...
def check_save(name: str, func, *args):
    try:
        ...
        if isinstance(data, list):
            rich.print(
                f"Found [bold red]{name}[/bold red] with the value [bold red]{[str(x) for x in data]}[/bold red]"
            )
        else:
            rich.print(
                f"Found [bold red]{name}[/bold red] with the value [bold red]{str(data)}[/bold red]"
            )
...
def location_data(
    address: str, site_designation: SiteDesignation, seismic_value: int | SiteClass
):
...
def building_dimensions(
    width: float, height: float = None, eave_height=None, ridge_height=None
):
...
def building(
    ...
    num_floor: int,
    h_opening: float,
    material_load: List[float] | float,
...
d88P     888  "Y8888P"  888        8888888888 888    Y888 88888888 "Y88888P"   "Y8888P88                                                                                                                                                                                                                                                                                            
    ...
    # site designation
    site_designation = check_save(
        "site_designation", choice, "site designation type", SiteDesignation
    )
    ...
    location = location_data(
        address=address, site_designation=site_designation, seismic_value=seismic_value
    )
    ...
    confirm_eave_ridge_choice = check_save(
        "confirm_eave_ridge_choice",
        confirm_choice,
        "Does the building have eave and ridge?",
    )
    ...
    confirm_height_zone = check_save(
        "confirm_height_zone",
        confirm_choice,
    ...
    dominant_opening = check_save(
        "dominant_opening", confirm_choice, "Does the building have Dominant Opening?"
    )

    if dominant_opening:
        mid_height = float(
            check_save("mid_height", user_input, "mid-height of the dominant opening")
        )
    else:
        mid_height = 0.0
    ...
    if not confirm_height_zone:
        custom_num_height_zones = int(
            check_save("custom_num_height_zones", user_input, "number of height zones")
        )
        if check_save_simple("height_zones") and confirm_choice(
            f"Would you like to keep them?"
        ):
            ...
            for i in range(custom_num_height_zones):
                height_zones.append(
                    HeightZone(
                        zone_num=i + 1,
                        elevation=float(
                            user_input(f"Elevation of height zone {i + 1}")
                        ),
                    )
                )
    ...
    w_roof = float(
        check_save("w_roof", user_input, "smaller plan dimension of the roof")
    )
    l_roof = float(
        check_save("l_roof", user_input, "larger plan dimension of the roof")
    )
    slope = float(check_save("slope", user_input, "slope of the roof"))

    confirm_material = check_save(
        "confirm_material",
        confirm_choice,
        "The material will be applied to all height zones?",
    )
    material_list = None
    wp = None
    if confirm_material:
        ...
        num_materials = int(
            check_save(
                "num_materials",
                user_input,
                "number of materials (same as number of height zones)",
            )
        )
        if check_save_simple("material_list") and confirm_choice(
            f"Would you like to keep them?"
        ):
            ...
            for i in range(num_materials):
                material_list.append(
                    float(user_input(f"material load for height zone {i + 1}"))
                )
    ...
    dimensions = building_dimensions(
        width=width, height=height, eave_height=eave_height, ridge_height=ridge_height
    )
    cladding = building_cladding(c_top=top_cladding, c_bot=bottom_cladding)
    roof = building_roof(
        w_roof=w_roof, l_roof=l_roof, slope=slope, uniform_dead_load=wp_roof
    )
    ...
    importance_category = check_save(
        "importance_category", choice, "importance category", ImportanceFactor
    )
    ...
    for height_zone in building.height_zones:
        ...
        confirm_topographic_factor = check_save(
            f"confirm_topographic_factor_{height_zone.zone_num}",
            confirm_choice,
            "Would you like to use a custom topographic factor (default 1)?",
        )
        topographic_factor = None
        if confirm_topographic_factor:
            topographic_factor = float(
                check_save(
                    f"topographic_factor_{height_zone.zone_num}",
                    user_input,
                    "topographic factor",
                )
            )
        ...
        exposure_factor_selection = check_save(
            f"exposure_factor_selection_{height_zone.zone_num}",
            choice,
        ...
        if exposure_factor_selection == WindExposureFactorSelections.INTERMEDIATE:
            manual_ce_cei = float(
                check_save(
                    f"exposure_factor_manual_value_{height_zone.zone_num}",
                    user_input,
                    "single value for both ce and cei",
                )
            )
            get_wind_exposure_factor(
                wind_factor_builder,
        ...
        internal_pressure_selection = check_save(
            f"internal_pressure_selection_{height_zone.zone_num}",
            choice,
    ...
    ar = None
    confirm_ar = check_save(
        "confirm_ar", confirm_choice, "Would you like to use the default ar value of 1?"
    )
    if confirm_ar:
        ar = 1
        serialize("ar", ar)
    ...
    rp = None
    confirm_rp = check_save(
        "confirm_rp",
        confirm_choice,
        "Would you like to use the default rp value of 2.5?",
    )
    if confirm_rp:
        rp = 2.5
        serialize("rp", rp)
    ...
    cp = None
    confirm_cp = check_save(
        "confirm_cp", confirm_choice, "Would you like to use the default cp value of 1?"
    )
    if confirm_cp:
        cp = 1
        serialize("cp", cp)
    ...
    uls_for_wall = check_save(
        "uls_for_wall", choice, "ULS for wall", ULSWallLoadCombinationTypes
    )
    sls_for_wall = check_save(
        "sls_for_wall", choice, "SLS for wall", SLSWallLoadCombinationTypes
    )
    zone_num = user_input(
        "Enter the zone number for which you want to compute the wall load combinations"
    )
    with pd.option_context(
        ...
        print(
            compute_wall_load_combinations(
                building, snow_load_downwind, uls_for_wall, sls_for_wall
            )
        )

    print("ROOF LOAD COMBINATIONS")
    uls_for_roof = check_save(
        "uls_for_roof", choice, "ULS for roof", ULSRoofLoadCombinationTypes
    )
    sls_for_roof = check_save(
        "sls_for_roof", choice, "SLS for roof", SLSRoofLoadCombinationTypes
    )
    with pd.option_context(
        "display.max_rows", None, "display.max_columns", None
    ):  # more options can be specified also
        print(
            compute_roof_load_combinations(
                building, snow_load_downwind, uls_for_roof, sls_for_roof
            )
        )
